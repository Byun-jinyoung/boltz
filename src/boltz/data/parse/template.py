...
class TemplateConstraintGenerator:
    ...
    def _calculate_sequence_identity(
        self, 
        aligned_seq1: str, 
        aligned_seq2: str
    ) -> float:
    ...
    def generate_template_constraints(
        ...
        constraint_type: str = "min_distance"
    ) -> List[Dict[str, Any]]:
        """
        ...
        constraint_type : str
            Type of constraint to generate ("min_distance")
        ...
        try:
            ...
            if seq_identity < self.min_sequence_identity:
                warnings.warn(
                    f"Low sequence identity ({seq_identity:.2f}). "
                    f"Constraints may not be reliable."
                )
            ...
            if not cb_coords:
                raise ValueError("No Cb coordinates extracted from template")
            ...
            print(f"  INFO: generated {len(unique_constraints)} template-based constraints")
